"""
worker_tasks.py  — Cobalt edition
===================================
Only ONE line changed from the original (marked ▼ below).
Everything else — db calls, Gemini upload, pipeline jobs — is untouched.
"""
...
# ▼ THE ONLY CHANGE: swap the import source
# Before: from ..routers.video_downloader import VideoDownloader
from ..cobalt_downloader import VideoDownloader          # ← Cobalt backend
# ▲ ─────────────────────────────────────────────────────────────────────────
...
def run_ingest_task(*, project_id: int, job_id: str, video_url: str, project_name: str | None) -> None:
    ...
    try:
        ...
        if not project:
            update_pipeline_job(job_id=job_id, status="failed", error="Project not found", message="Project not found")
            return
        ...
        # Temporarily redirect the downloader to /tmp (same pattern as original)
        ...
        try:
            _comments_extractor.extract_comments(url=video_url, max_comments=100, include_replies=True)
        ...
        if not project.video_id:
            video = Video(
                ...
                title=project_name or project.name or "Analyzed Video",
                description=f"Video analysis for project: {project_name or project.name}",
                platform="youtube",
                url=video_url,
                duration_seconds=30,
        ...
        update_pipeline_job(job_id=job_id, status="completed", step=5, message="Opening Streamline", project_id=project_id)
        ...
        update_pipeline_job(job_id=job_id, status="failed", message="Pipeline failed", error=str(exc), project_id=project_id)
        try:
            project = db.query(Project).filter(Project.id == project_id).first()
            if project:
...
def run_overview_task(*, project_id: int) -> None:
    ...
    try:
        ...
        overview = db.query(ProjectOverview).filter(ProjectOverview.project_id == project_id).first()
        if not overview:
