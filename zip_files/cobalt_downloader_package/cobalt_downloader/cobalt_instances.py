...
It is safe to instantiate once at module level (singleton pattern) — the
...
# Instances may come and go — the tester will weed out dead ones automatically.
# ─────────────────────────────────────────────────────────────────────────────
        ...
        try:
            async with session.get(
                ...
                if resp.status in (401, 403):
                    ...
                    result["error"] = f"HTTP {resp.status} — auth required"
...
# ─────────────────────────────────────────────────────────────────────────────
# Public rotator — consumed by VideoDownloader
# ─────────────────────────────────────────────────────────────────────────────
class CobaltInstanceRotator:
    ...
    async def refresh_async(self) -> list[dict]:
        """Async refresh — use inside FastAPI lifespan or async context."""
        logger.info("CobaltInstanceRotator: probing %d instances…", len(self._all))
    ...
    def refresh_sync(self) -> list[dict]:
        ...
        try:
            ...
            if loop.is_running():
                ...
                nest_asyncio.apply()
                return loop.run_until_complete(self.refresh_async())
            return loop.run_until_complete(self.refresh_async())
        except RuntimeError:
    ...
    def print_status(self) -> None:
        ...
        for r in self._ranked:
            if not r["online"]:
                continue
            rank += 1
            auth = "🔒" if r["requires_auth"] else "open"
            print(f"{'★ ' if rank <= 5 else '  '}{rank:<3} {r['latency_ms']:>6.0f} ms  "
