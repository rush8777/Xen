"""
Video downloader module for direct integration
Can be imported and used without running as CLI
"""

import yt_dlp
import os
from pathlib import Path
from typing import Dict, Optional


class VideoDownloader:
    """
    Video downloader service using yt-dlp
    Supports YouTube, Facebook, TikTok, Instagram, and many other platforms
    """
    
    def __init__(self, download_dir: str = "./downloads"):
        """
        Initialize the video downloader
        
        Args:
            download_dir: Directory to save downloaded videos
        """
        self.download_dir = Path(download_dir)
        self.download_dir.mkdir(exist_ok=True)
        
    def get_info(self, url: str) -> Dict:
        """
        Get video information without downloading
        
        Args:
            url: Video URL
            
        Returns:
            Dictionary containing video metadata
        """
        ydl_opts = {
            'quiet': True,
            'no_warnings': True,
            'extract_flat': False,
        }
        
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(url, download=False)
            return info
    
    def download(
        self,
        url: str,
        quality: str = "best",
        audio_only: bool = False,
        output_filename: Optional[str] = None
    ) -> str:
        """
        Download video and return filepath
        
        Args:
            url: Video URL
            quality: Video quality (best, 1080p, 720p, 480p, 360p)
            audio_only: If True, extract audio only
            output_filename: Optional custom output filename (without extension)
            
        Returns:
            Path to downloaded file
        """
        
        # Quality format mapping for video
        # TODO: Revert to original format mapping after ffmpeg is installed
        # Current mapping avoids ffmpeg requirement by using single-file formats only
        quality_map = {
            "best": "best[ext=mp4]/best",  # Avoids merging, uses single file
            "1080p": "best[height<=1080][ext=mp4]/best[height<=1080]/best",
            "720p": "best[height<=720][ext=mp4]/best[height<=720]/best", 
            "480p": "best[height<=480][ext=mp4]/best[height<=480]/best",
            "360p": "best[height<=360][ext=mp4]/best[height<=360]/best",
        }
        
        # Determine output template
        if output_filename:
            outtmpl = str(self.download_dir / f"{output_filename}.%(ext)s")
        else:
            outtmpl = str(self.download_dir / '%(title)s.%(ext)s')
        
        # Base options
        ydl_opts = {
            'format': 'bestaudio/best' if audio_only else quality_map.get(quality, quality_map["best"]),
            'outtmpl': outtmpl,
            'quiet': True,
            'no_warnings': True,
            'socket_timeout': 300,  # 5 minutes
        }
        
        # Add audio extraction post-processor if needed
        # TODO: Uncomment after ffmpeg is installed
        if audio_only:
            # ydl_opts['postprocessors'] = [{
            #     'key': 'FFmpegExtractAudio',
            #     'preferredcodec': 'mp3',
            #     'preferredquality': '192',
            # }]
            # # Ensure we have ffmpeg
            # ydl_opts['prefer_ffmpeg'] = True
            pass  # Skip audio extraction for now
        
        # Download the video
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(url, download=True)
            
            # Get the downloaded filename
            filename = ydl.prepare_filename(info)
            
            # Handle case where prepare_filename returns None
            if filename is None:
                # Fallback: construct filename from info
                if output_filename:
                    filename = str(self.download_dir / f"{output_filename}.mp4")
                else:
                    # Sanitize title and create filename
                    safe_title = "".join(c for c in info.get('title', 'video') if c.isalnum() or c in (' ', '-', '_')).rstrip()
                    filename = str(self.download_dir / f"{safe_title}.mp4")
            
            # Handle audio extraction filename change
            # TODO: Uncomment after ffmpeg is installed
            if audio_only:
                # yt-dlp changes extension to .mp3 after extraction
                # filename = filename.rsplit('.', 1)[0] + '.mp3'
                pass  # Keep original extension for now
            
            return filename
    
    def get_supported_platforms(self) -> list:
        """
        Get list of supported platforms
        Note: yt-dlp supports 1000+ sites, here are the main ones
        """
        return [
            "YouTube",
            "Facebook",
            "Instagram",
            "TikTok",
            "Twitter/X",
            "Vimeo",
            "Dailymotion",
            "Reddit",
            "Twitch",
            "LinkedIn",
            # ... and many more
        ]
