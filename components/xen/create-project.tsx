...
export default function CreateProjectModal({
  ...
  const [modalSize, setModalSize] = useState<'max-w-4xl' | 'max-w-md'>('max-w-md')
  ...
  useEffect(() => {
    const updateModalSize = () => {
      // Calculate available width considering sidebar state
      const sidebarWidth = isSidebarExpanded ? 256 : 80 // Approximate sidebar widths
  ...
  }, [isSidebarExpanded]) // Add sidebar state as dependency
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative w-full max-h-[85vh] p-3 rounded-lg bg-white dark:bg-zinc-900/70 border border-zinc-100 dark:border-zinc-800 shadow-sm backdrop-blur-xl flex flex-col transition-all duration-500 ease-in-out ${modalSize}`}>
              ...
              <div className="flex-shrink-0 px-4 pb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative platform-dropdown">
                    ...
                    {/* Dropdown Menu */}
                    {isPlatformDropdownOpen && (
                      <div className="absolute top-full left-0 mt-1 w-40 bg-[#35353A] border border-[#4A4A4E] rounded-lg shadow-lg z-50">
                        {platforms.map((platform) => (
                          <button
                            key={platform}
                            type="button"
                            onClick={() => handlePlatformSelect(platform)}
                            className="w-full px-3 py-2 text-[10px] text-left text-white hover:bg-[#4A4A4E] transition-colors first:rounded-t-lg last:rounded-b-lg"
                          >
                            {platform}
                          </button>
                        ))}
                ...
                <div className="grid grid-cols-4 gap-3">
                  ...
                  {!videosLoading && filteredVideos.map((video, index) => (
                    <div 
                      key={video.id || index}
                      className={
                        "group cursor-pointer animate-in fade-in zoom-in-95 duration-300 " +
        ...
        {/* Footer */}
        <div className="flex items-center justify-between p-4 pt-2 border-t border-zinc-800 flex-shrink-0">
          <button
            type="button"
            onClick={onClose}
            className="px-3 py-1.5 text-xs font-medium text-gray-400 hover:text-white hover:bg-[#4A4A4E] rounded-lg transition-colors duration-200"
          >
            Cancel
          </button>
          <button
            ...
            className="px-4 py-1.5 text-xs font-semibold text-white bg-purple-600 hover:bg-purple-700 rounded-lg transition-colors duration-200 flex items-center gap-1.5"
          >
            Create Project
          </button>
        </div>
